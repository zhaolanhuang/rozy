...
pub fn render_html(analysis: &Analysis) -> Result<String, RosyError> {
    let json = serde_json::to_string(analysis)?
        .replace('&', "\\u0026")
        .replace('<', "\\u003c")
        .replace('>', "\\u003e");
    Ok(TEMPLATE.replace("__ROSY_REPORT_JSON__", &json))
}

/// Render and write an analysis as a self-contained, offline HTML report.
pub fn write_html(path: impl AsRef<Path>, analysis: &Analysis) -> Result<(), RosyError> {
    let path = path.as_ref();
    fs::write(path, render_html(analysis)?).map_err(|source| RosyError::Write {
    ...
    fn embeds_crate_grouping_and_attribution() {
        let analysis = Analysis {
            ...
            symbols: vec![SymbolReport {
                ...
                section: ".text".into(),
                path: vec!["rosy".into(), "src".into(), "main.rs".into()],
                crate_name: Some("rosy".into()),
                source: Some("rosy/src/main.rs".into()),
                line: Some(1),
                synthetic: false,
            }],
        };
        ...
        assert!(html.contains("\"crate_name\":\"rosy\""));
